...
namespace SoleAuthenticity_Ver2.Controllers
{
    ...
    public class BrandController : ControllerBase
    {
        ...
        [HttpGet("cus/pagination", Name = "GetBrandsVerCusWithPagination")]
        [Produces("application/json")]
        [ProducesResponseType((int)HttpStatusCode.OK)]
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        public async Task<ActionResult<ServiceResponse<IEnumerable<BrandDtoVerCus>>>> GetBrandsVerCusWithPagination([FromQuery] int page, [FromQuery] int pageSize)
        {
            try
            {
                var res = await _brandservice.GetAllBrandsVerCusWithPagination(page, pageSize);
                return StatusCode((int)res.StatusCode, res);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("cus/pagination/count", Name = "CountBrandsVerCusWithPagination")]
        [Produces("application/json")]
        [ProducesResponseType((int)HttpStatusCode.OK)]
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        public async Task<ActionResult<ServiceResponse<int>>> CountBrandsVerCusWithPagination()
        {
            try
            {
                var res = await _brandservice.CountAllBrandsVerCusWithPagination();
                return StatusCode((int)res.StatusCode, res);
            }
            catch (Exception ex)
            {
